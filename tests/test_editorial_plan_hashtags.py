...
def test_editorial_plan_audit_limits_hashtags_and_detects_invalid_format():
    profile, market, compatibility, audit_report = build_editorial_plan_inputs()
    plan = replace_editorial_post(
        build_editorial_plan(),
        0,
        hashtags=["#Agile", "#Jira", "#Stakeholders", "#Liderazgo", "#Tecnologia", "sin-hash"],
    )

    audit = audit_editorial_plan(plan, profile, market, compatibility, audit_report)

    assert audit.passed is False
    assert any("máximo 5" in finding.message or "Hashtag inválido" in finding.message for finding in audit.findings)
